...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      title: `Room preview · ${artwork.title}`,
      description: `See how ${artwork.title} looks on living-room, office, salon, gallery, and restaurant walls.`,
      robots: { index: false, follow: true },
    };
  } catch {
    return { title: "Room preview", robots: { index: false } };
  }
}

export default async function ArtworkMockupsPage({
  params,
}: {
  params: Promise<{ slug: string; artworkId: string }>;
}) {
  ...
  return (
    <RoomMockupViewer
      ...
      spaceHref={`/g/${manifest.slug}/a/${artwork.id}/space`}
    />
  );
}
